...
def build_prices_json(universe, raw_data, benchmark_rows):
    ...
    for stock in universe["stocks"]:
        ...
        # 80-day-ago 200D MA (Stage 1 gate: 200D declining vs M-4; Stage 3 gate: 200D rising vs M-4)
        ...
        # MD-V2-S54-MARKER: soft-stack streak (Stage 1 Plausible/Probable metric)
        # Consecutive trading days backward from today where sma_50 >= 0.97×sma_150 AND sma_150 >= 0.97×sma_200.
        soft_stack_streak = 0
        for _sr in reversed(rows_with_sma):
            _s50 = _sr.get("sma_50")
            _s150 = _sr.get("sma_150")
            _s200 = _sr.get("sma_200")
            if _s50 is None or _s150 is None or _s200 is None:
                break
            if _s50 >= 0.97 * _s150 and _s150 >= 0.97 * _s200:
                soft_stack_streak += 1
            else:
                break
        ...
        entry = {
            ...
            # MD-V2-S54-MARKER: new fields for Stage 1/3 rewrite
            "soft_stack_streak": soft_stack_streak,
...
def compute_master_dashboard_screens(prices, filter_results):
    ...
    for fr in filter_results:
        ...
        # ──────────────────────────────────────────────────────────────
        # STAGE 1 — Consolidating / Basing  [MD-V2-S54-REWRITE]
        # Gate 1: 200D MA today < 200D MA 80 trading days ago (MA in decline)
        # Gate 2: Price > 150D MA (still above medium-term support)
        # Metric: soft-stack streak (consecutive days where 50D >= 0.97×150D AND 150D >= 0.97×200D)
        # Plausible: streak >= 20 days; Probable: streak >= 60 days; None: < 20 days
        # ──────────────────────────────────────────────────────────────
        s1 = {"tests": {}, "groups": {}, "count": 0, "rating": "None"}
        _soft_streak = p.get("soft_stack_streak", 0) or 0
        s1_gate1 = (ma200 is not None and ma200_80d_ago is not None and ma200 < ma200_80d_ago)
        s1_gate2 = (price is not None and ma150 is not None and price > ma150)
        s1["gate_200D_declining_vs_80d"] = s1_gate1
        s1["gate_price_above_150D"] = s1_gate2
        s1["streak"] = _soft_streak
        s1["test_values"] = {
            "gate_200D_vs_80d_ago": _md_v2_pct_gap(ma200, ma200_80d_ago),
            "gate_price_vs_150D": _md_v2_pct_gap(price, ma150),
            "soft_stack_streak_days": _soft_streak,
        }
        if s1_gate1 and s1_gate2:
            if _soft_streak >= 60:
                s1["rating"] = "Probable"
            elif _soft_streak >= 20:
                s1["rating"] = "Plausible"
            else:
                s1["rating"] = "None"
        else:
        ...
        persistence = {
            "stage_1_persistence": [False] * 11 + [s1["rating"] in ("Plausible", "Probable")],
