"""
=============================================================================
PATCHER — tests_speculative_bet dashboard tab (Speculative Bets: Stages 3+4)
=============================================================================
Project: SA - Master Dashboard
Session: S47 (18-May-2026), Task 2 (§6.1)
Decision: D-MD-V2-109 + D-MD-V2-111 (locked 18-May-2026)
...
----------------------
Adds a new dashboard tab "tests_speculative_bet" rendering two S46 pipeline
test variants side-by-side:
  - speculative_bet_s3 (Stage 3 topping/declining speculative bet) — 6 criteria
  - speculative_bet_s4 (Stage 4 declining/basing speculative bet) — 6 criteria
...
Changes made to build_dashboard.py:
  ...
  5. CSS block for #tab-tests_speculative_bet
  6. Self-contained render module (IIFE) — two patterns on one tab
  7. renderTab dispatcher case
  8. CSS selectors for V2 tab chrome

DEPENDS ON: Patcher 2 (tests_probing_bet) having been applied first,
because this patcher's anchors include MARKER text from Patcher 2.

USAGE
-----
1. Dry-run:  python3 scripts/patch_md_v2_tab_speculative_bet_s47_2026_05_18.py --test
2. Apply:    python scripts/patch_md_v2_tab_speculative_bet_s47_2026_05_18.py
             python scripts/build_dashboard.py
             git add scripts/build_dashboard.py index.html
             git commit -m "feat(MD V2 S47): speculative-bet tab S3+S4 (6-criterion x2, S46 pipeline)"
=============================================================================
"""
...
MARKER         = "MD-V2-S47-TAB-SPECULATIVE-BET-MARKER"
# Patcher 2 must have been applied first
PREREQ_MARKER  = "MD-V2-S47-TAB-PROBING-BET-MARKER"
BAK_TAG        = "s47-tab-sb"
ENABLE_PY_COMPILE = True
# ======================================================================

# 1. TABS array: insert after tests_probing_bet entry
ANCHOR_1 = '    {"id": "tests_probing_bet", "label": "Probing Bets", "accent": "#6b46c1"},'
REPLACE_1 = ('    {"id": "tests_probing_bet", "label": "Probing Bets", "accent": "#6b46c1"},\n'
...
# 2. IMPLEMENTED_TABS: insert after tests_probing_bet
ANCHOR_2 = '    "tests_probing_bet",  # MD-V2-S47-TAB-PROBING-BET-MARKER'
REPLACE_2 = ('    "tests_probing_bet",  # MD-V2-S47-TAB-PROBING-BET-MARKER\n'
             '    "tests_speculative_bet",  # MD-V2-S47-TAB-SPECULATIVE-BET-MARKER')

# 3. Nav button: insert after probing bet button
ANCHOR_3 = "      + '<button class=\"v2-nav-btn v2-grp-tests\" data-v2-tab=\"tests_probing_bet\" onclick=\"switchTab(\\'tests_probing_bet\\')\">Probing Bets</button>'  /* MD-V2-S47-TAB-PROBING-BET-MARKER */"
REPLACE_3 = ("      + '<button class=\"v2-nav-btn v2-grp-tests\" data-v2-tab=\"tests_probing_bet\" onclick=\"switchTab(\\'tests_probing_bet\\')\">Probing Bets</button>'  /* MD-V2-S47-TAB-PROBING-BET-MARKER */\n"
...
ANCHOR_4 = 'var _v2chartTabs={stage_1:1,stage_2:1,stage_3:1,stage_4:1,pre_indicators:1,post_indicators:1,setups_s1pb:1,setups_s2vcp:1,tests:1,setups_healthy_retest:1,tests_probing_bet:1,master_overview:1};  /* MD-V2-S47-TAB-PROBING-BET-MARKER */'
REPLACE_4 = 'var _v2chartTabs={stage_1:1,stage_2:1,stage_3:1,stage_4:1,pre_indicators:1,post_indicators:1,setups_s1pb:1,setups_s2vcp:1,tests:1,setups_healthy_retest:1,tests_probing_bet:1,tests_speculative_bet:1,master_overview:1};  /* MD-V2-S47-TAB-SPECULATIVE-BET-MARKER */'

# 5. Chart wiring _v2ct
ANCHOR_5 = 'var _v2ct={stage_1:1,stage_2:1,stage_3:1,stage_4:1,pre_indicators:1,post_indicators:1,setups_s1pb:1,setups_s2vcp:1,tests:1,setups_healthy_retest:1,tests_probing_bet:1,master_overview:1};  /* MD-V2-S47-TAB-PROBING-BET-MARKER */'
REPLACE_5 = 'var _v2ct={stage_1:1,stage_2:1,stage_3:1,stage_4:1,pre_indicators:1,post_indicators:1,setups_s1pb:1,setups_s2vcp:1,tests:1,setups_healthy_retest:1,tests_probing_bet:1,tests_speculative_bet:1,master_overview:1};  /* MD-V2-S47-TAB-SPECULATIVE-BET-MARKER */'

# 6. renderTab dispatcher: insert after probing bet case
ANCHOR_6 = "  else if(id===\"tests_probing_bet\")renderProbingBet();  /* MD-V2-S47-TAB-PROBING-BET-MARKER */"
REPLACE_6 = ("  else if(id===\"tests_probing_bet\")renderProbingBet();  /* MD-V2-S47-TAB-PROBING-BET-MARKER */\n"
...
def main(argv):
    ...
    if not test_mode:
        if PREREQ_MARKER not in wt_src:
            ...
            print(f"        Apply patch_md_v2_tab_probing_bet_s47_2026_05_18.py first.")
            return 2
        src = wt_src
    else:
        if PREREQ_MARKER in wt_src:
            src = wt_src
            print(f"[*] Prerequisite marker found in WT (Patcher 2 applied, not committed). Using WT source.")
    ...
    if n7 != 1:
        ...
        return 3
    src = src.replace(ANCHOR_7, SB_CSS + "\n" + SB_MODULE + "\n" + ANCHOR_7, 1)
