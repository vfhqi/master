"""
=============================================================================
PATCH MD V2 -- CHART ZOOM OVERRIDE PER TAB (Session 42, Item #13, Path A)
=============================================================================
Layers a per-tab zoom override on top of the existing two-tier default in
window.openChart (build_dashboard.py).

Spec (Path A -- per-tab, signed off by Richard 17-May-26):
- post_indicators tab  -> 2Y zoom on chart open (200D break-down lives here)
- setups_s1pb tab      -> 3M zoom on chart open (vcp_after_s1_plateau here)
- setups_s2vcp tab     -> 3M zoom on chart open (vcp_after_s2_base here)
- All other non-Stage tabs -> 6M (existing default, unchanged)
- All Stage tabs           -> 2Y (existing default, unchanged)

Target: scripts/build_dashboard.py, inside window.openChart, line ~2707
where chartZoom is set on fresh-open from V2 tabs.

Design fork notes: PROJECTS/SA - Master Dashboard/
                   session-42-item13-chart-zoom-override-design-fork-2026-05-17.md
=============================================================================
"""
...
MARKER         = "MD-V2-CHART-ZOOM-PER-TAB-OVERRIDE-S42-MARKER"
BAK_TAG        = "chart-zoom-per-tab-override-s42"
ENABLE_PY_COMPILE = True
# ======================================================================

# ============ EDIT: the actual textual change =========================
ANCHOR = '''    chartZoom=_isStageChart?"2Y":"6M";'''

REPLACEMENT = '''    // MD-V2-CHART-ZOOM-PER-TAB-OVERRIDE-S42-MARKER: per-tab zoom override layered above the two-tier default. 200D break-down on post_indicators; vcp_after_s1_plateau on setups_s1pb; vcp_after_s2_base on setups_s2vcp.
...
def main(argv):
    ...
    assert MARKER in new_src, "[INTERNAL] MARKER missing after replace -- check REPLACEMENT contains MARKER as substring"
